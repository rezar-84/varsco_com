  ...
  const renderInline = (text: string) => {
    ...
    return parts.map((part, idx) => {
      ...
      if (linkMatch) {
        ...
        // Blog content is authored internally today, but this renders
        // whatever markdown it's given — reject javascript:/data: etc. so a
        // future CMS-sourced or user-influenced post can't smuggle a script
        // URL through a link.
        const isSafeScheme = href.startsWith("/") || /^https?:\/\//i.test(href);
        if (!isSafeScheme) {
          return label;
        }
        if (href.startsWith("/")) {
      ...
      {blocks.map((block, blockIdx) => {
        ...
        if (trimmed.startsWith("![") && trimmed.includes("](") && trimmed.endsWith(")")) {
          ...
          if (match) {
            ...
            return (
              <div
                key={blockIdx}
                className="my-6 overflow-hidden rounded-2xl border border-border/80 bg-background shadow-md max-w-2xl mx-auto"
              >
        ...
        if (trimmed.startsWith("##")) {
          return (
            <h2
              key={blockIdx}
              className="font-display text-2xl font-bold text-navy mt-8 mb-3 border-b pb-2"
            >
              {renderInline(trimmed.replace(/^##\s+/, ""))}
            </h2>
          );
                  ...
                  <thead>
                    <tr className="bg-navy/5 border-b border-white/20">
                      {headerCells.map((h, idx) => (
                        <th
                          key={idx}
                          className="p-3 text-xs font-bold text-navy uppercase tracking-wider"
                        >
                          {h}
                        </th>
                      ))}
        ...
        if (
          trimmed.includes("Rotifer Yield") &&
          trimmed.includes("350%") &&
          trimmed.includes("800%")
        ) {
          return (
            <div
              key={blockIdx}
              className="my-8 rounded-3xl border border-white/20 bg-gradient-to-br from-navy to-navy-light p-6 text-white shadow-xl relative overflow-hidden"
            >
              ...
              <h4 className="font-display text-lg font-bold text-white mb-2">
                Rotifer Sourcing Efficiency Comparison
              </h4>
              <p className="text-xs text-white/70 mb-6">
                Comparison of rotifer production yield in a standard 3-day culture cycle.
              </p>
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between text-xs font-semibold mb-2">
                    <span className="text-white/80">
                      Standard Industry Production (Before Optimization)
                    </span>
                ...
                <div>
                  <div className="flex justify-between text-xs font-semibold mb-2">
                    <span className="text-white/80">
                      VARS Algae Protocol (Fresh Chlorella SV12)
                    </span>
                    <span className="text-mint font-bold">800% (8.0x Yield)</span>
                  </div>
                  <div className="w-full bg-white/10 h-3 rounded-full overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-mint to-teal-400 h-full rounded-full transition-all duration-1000 shadow-[0_0_12px_rgba(45,212,191,0.3)]"
                      style={{ width: "80%" }}
                    />
                  </div>
                </div>
        ...
        if (
          trimmed.includes("Price vs. Performance") ||
          (trimmed.includes("Artemia") &&
            trimmed.includes("Viable larva") &&
            trimmed.includes("US Artemia"))
        ) {
          return (
            <div
              key={blockIdx}
            ...
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-mint/10 rounded-full blur-2xl pointer-events-none" />
              <h4 className="font-display text-lg font-bold text-white mb-2">
                Artemia Sourcing Cost-to-Performance Index
              </h4>
              <p className="text-xs text-white/70 mb-6">
                Normalized cost per viable surviving larva (Lower index indicates superior
                economics).
              </p>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex justify-between text-xs font-semibold mb-2">
                    <span className="text-white/80">
                      Aral Sea Artemia + Fresh Chlorella SV12 Protocol
                    </span>
